...
// Each facet has a distinct column layout — textCls/chartCls are full Tailwind strings
// so JIT includes them. flip reverses column order on desktop.
const FACETS = [
  {
    to: '/tax-impact',
    cat: 'Fiscal Impact',
    ...
    // Text narrow, chart wide — allocation bars earn visual dominance
  ...
  },
  {
    ...
    // Equal columns, chart moves left — breaks the repeating pattern
  ...
  },
  {
    ...
    // Chart wide and left — energy chart is the most visually distinct
  ...
  },
  {
    ...
    catBorder: 'border-violet-500/30',
    ...
    // Text wide — precedent case needs narrative room; chart supports on the right
...
export default function Home() {
  return (
    <div>
      ...
      {/* ── Hero ─────────────────────────────────────────────────────────────── */}
      <section className="relative bg-grid min-h-[92vh] flex flex-col justify-center overflow-hidden">

        <div
          className="absolute inset-0 pointer-events-none"
          style={{
            background: 'radial-gradient(ellipse 70% 60% at 60% 40%, rgba(14,165,233,0.05) 0%, transparent 70%)',
          }}
        />
          ...
          <div className="grid lg:grid-cols-12 gap-12 lg:gap-8 items-center">
            ...
            <div className="lg:col-span-7">
              <motion.div initial="hidden" animate="visible" variants={heroLines}>
                ...
                <motion.p
                  variants={heroLine}
                  className="text-2xs font-mono text-blue-400/60 uppercase tracking-[0.2em] mb-6 flex items-center gap-2"
                >
                ...
                <h1 className="text-5xl sm:text-6xl lg:text-7xl font-display font-bold leading-none mb-8">
                  ...
                  <motion.span variants={heroLine} className="block text-gray-50">$8.5–18B</motion.span>
                  <motion.span variants={heroLine} className="block text-gray-100">AI campus</motion.span>
                  <motion.span variants={heroLine} className="block text-gray-400">reshaping</motion.span>
                  <motion.span variants={heroLine} className="block text-gray-100">a village.</motion.span>
                </h1>

                <motion.p variants={heroLine} className="text-base text-gray-400 max-w-lg leading-relaxed mb-8">
                ...
                <motion.div variants={heroLine} className="flex flex-wrap gap-4">
                  <Link
                    to="/timeline"
                    className="inline-flex items-center gap-2 px-5 py-2.5 rounded border border-blue-500/40 text-blue-300 hover:border-blue-400/70 hover:text-blue-200 text-sm font-medium transition-all duration-150"
                  >
                    View timeline
                  ...
                  <Link
                    to="/sources"
                    className="inline-flex items-center gap-2 px-5 py-2.5 rounded border border-gray-700 hover:border-gray-500 text-gray-400 hover:text-gray-200 text-sm font-medium transition-all duration-150"
                  >
                    Primary sources
                  </Link>
                </motion.div>
            ...
            <div className="lg:col-span-5">
              <div className="space-y-3">
                ...
                {/* Primary stat */}
                <FadeIn delay={0.4} className="bg-gray-900/50 border border-gray-800/60 rounded-xl p-6 backdrop-blur-sm">
                  <p className="text-2xs font-mono text-gray-500 uppercase tracking-widest mb-3">IT Capacity at Full Buildout</p>
                  <div className="flex items-end gap-2">
                    <AnimatedNumber
                      value={project.totalCapacityMW}
                      delay={0.5}
                      duration={1.8}
                      className="text-6xl font-display font-bold text-blue-400 leading-none tracking-tight"
                    />
                    <span className="text-2xl font-display font-bold text-blue-500/40 mb-1">MW</span>
                  </div>
                  <div className="flex items-center gap-2 mt-3">
                    <span className="text-2xs font-mono text-gray-500">+{project.securedPowerMW - project.totalCapacityMW} MW buffer</span>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    { label: 'Site area',      value: project.totalAcres,  suffix: ' ac', color: 'text-gray-100', delay: 0.55 },
                    { label: 'Perm. jobs',     value: jobs.permanent,       suffix: '',    color: 'text-gray-100', delay: 0.6  },
                    { label: 'Max buildings',  value: project.maxBuildings, suffix: '',    color: 'text-gray-100', delay: 0.65 },
                    { label: 'Phase 1 online', value: null, static: project.firstBuildingOnline, color: 'text-gray-100', delay: 0.7 },
                  ].map(({ label, value, suffix, static: staticVal, color, delay }) => (
                    <FadeIn key={label} delay={delay} className="bg-gray-900/40 border border-gray-800/50 rounded-lg p-4">
                      <p className="text-2xs font-mono text-gray-500 uppercase tracking-widest mb-2">{label}</p>
                      {staticVal ? (
                        <p className={`text-2xl font-display font-bold ${color}`}>{staticVal}</p>
                      ) : (
                        <AnimatedNumber
                          value={value}
                          suffix={suffix}
                ...
                <FadeIn delay={0.75} className="text-2xs text-gray-500 text-right">
        ...
        >
          <span className="text-2xs font-mono text-gray-600 uppercase tracking-[0.2em]">Scroll</span>
          <motion.svg
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 pt-4 pb-14">
        <FadeIn>
          <div className="flex items-end justify-between mb-4">
            <div>
              <p className="text-2xs font-mono text-blue-400/50 uppercase tracking-[0.2em] mb-1">Site Location</p>
              <h2 className="text-2xl font-display font-bold text-gray-100">Peterson Rd &amp; Route 83</h2>
            </div>
            <Link to="/map" className="text-xs text-gray-500 hover:text-gray-200 transition-colors flex items-center gap-1">
              Full map
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 pb-20">
        ...
        <FadeIn className="mb-12">
          <p className="text-2xs font-mono text-gray-500 uppercase tracking-[0.2em] mb-3 flex items-center gap-2">
            <span className="inline-block w-4 h-px bg-gray-700" />
            Impact by category
          </p>
          <h2 className="text-3xl font-display font-bold text-gray-100">Four dimensions of impact.</h2>
            ...
            <FadeIn key={to} delay={i * 0.08}>
              <div className={`border-t ${catBorder} pt-10 pb-14 grid lg:grid-cols-12 gap-8 lg:gap-12`}>
                ...
                <div className={`${textCls}${flip ? ' lg:order-last' : ''}`}>
                  ...
                  <h3 className="text-2xl font-display font-bold text-gray-100 leading-tight mb-4">
                    {headline}
                  </h3>
                  {highlight && (
                    <div className="mb-5">
                      <AnimatedNumber
                        ...
                        duration={1.6}
                        className="text-5xl font-display font-bold tracking-tight text-gray-100"
                      />
                      <p className="text-2xs font-mono text-gray-500 uppercase tracking-widest mt-1">{highlight.label}</p>
                    </div>
                  )}
                  <p className="text-sm text-gray-400 leading-relaxed mb-5">{body}</p>
                ...
                <div className={`${chartCls}${flip ? ' lg:order-first' : ''}`}>
                  <p className="text-2xs font-mono text-gray-500 uppercase tracking-widest mb-5">{chartLabel}</p>
                  {chart}
                </div>
